...
public class WANBMFinishFrame extends JFrame{
	...
	public WANBMFinishFrame(WANBMFindEnemyFrame myfindEnemyFrame,int inputCode,int codeReceive,boolean isWinner) {
			...
			public void actionPerformed(ActionEvent e) {
				if(BattleModeICintinueReady==false) {
					...
					if(BattleModeEnemyCintinueReady) {	//如果对方已准备
						...
						TextDialog txtDl=new TextDialog(WANBMFinishFrame.this,"已准备","<html>已准备！<br/>请等待对方开始游戏</html>");
					...
					TextDialog txtDl=new TextDialog(WANBMFinishFrame.this,"取消准备","已取消准备！");
					Dimension screensize=Toolkit.getDefaultToolkit().getScreenSize();
					txtDl.setSize(600, 300); 
					txtDl.setLocation((int)screensize.getWidth()/2-300,(int)screensize.getHeight()/2-150);
					txtDl.setVisible(true);
